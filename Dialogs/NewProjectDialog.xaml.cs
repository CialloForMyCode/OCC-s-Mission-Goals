...
public partial class NewProjectDialog : UserControl
{
    ...
    public bool IsOpenMode => OpenModeBtn.IsChecked == true;
    ...
    public void Reset()
    {
        ...
        DialogTitle.Text = LocalizationManager.T("新建项目", "New Project");
    }
    ...
    public void PrepareOpen()
    {
        ...
        OpenHint.Text = string.Empty;
        DialogTitle.Text = LocalizationManager.T("打开项目", "Open Project");
        LoadProjects();
    }
    ...
    public void LoadConfig(string name, string description, string currentVersion)
    {
        ...
        NewPanel.Visibility = Visibility.Visible;
        OpenPanel.Visibility = Visibility.Collapsed;
        ConfirmBtn.Content = LocalizationManager.T("保存", "Save");
        ProjectNameTextBox.Text = name;
        DescriptionTextBox.Text = description;
        VersionTextBox.Text = currentVersion;
        DialogTitle.Text = LocalizationManager.T("设置项目", "Project Settings");
    }
    ...
    public (bool IsValid, string Message) Validate()
    {
        if (string.IsNullOrWhiteSpace(ProjectName))
            return (false, LocalizationManager.T("项目名称不能为空。", "Project name cannot be empty."));

        if (ProjectName.Length > 100)
            return (false, LocalizationManager.T("项目名称不能超过 100 个字符。", "Project name cannot exceed 100 characters."));

        if (Description.Length > 500)
            return (false, LocalizationManager.T("项目描述不能超过 500 个字符。", "Project description cannot exceed 500 characters."));
    ...
    private void ConfirmButton_Click(object sender, RoutedEventArgs e)
    {
        if (Confirmed == null)
        {
            MessageBox.Show(LocalizationManager.T("Confirmed 事件未绑定，请通过菜单「项目」打开此弹窗。", "Confirmed event is not bound. Open this dialog via the Project menu."),
                LocalizationManager.T("提示", "Notice"), MessageBoxButton.OK, MessageBoxImage.Warning);
            return;
        }
        ...
        {
            OpenHint.Text = LocalizationManager.T("请选择一个项目。", "Please select a project.");
            return;
        }
