...
6. **프로젝트 관리** — 설정 → 프로젝트 정보: 이름과 설명 편집 또는 현재 프로젝트 삭제(모든 버전과 항목 영구 삭제, 되돌릴 수 없음)
...
현재 프로젝트의 유형 태그(카테고리)를 관리합니다. 변경 사항은 모든 버전의 항목에 동기화됩니다.

| 사용법 | 설명 |
|--------|------|
| `tag list` | 모든 태그 나열 (색상 포함) |
| `tag add <이름> [--color <hex>]` | 태그 생성 (별칭 `new`) |
| `tag delete <이름>` | 태그 삭제 및 모든 버전 항목에서 제거 (별칭 `remove` / `rm`) |
| `tag rename <기존> <새>` | 태그 이름 변경 및 모든 버전 항목에 동기화 (별칭 `mv`) |
