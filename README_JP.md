...
6. **プロジェクト管理** — 設定 → プロジェクト情報：名前と説明を編集、または現在のプロジェクトを削除（全バージョンとエントリを完全に削除、元に戻せません）
...
現在のプロジェクトのタイプタグ（カテゴリ）を管理します。変更はすべてのバージョンのエントリに同期されます。

| 使用法 | 説明 |
|--------|------|
| `tag list` | すべてのタグを一覧表示（色付き） |
| `tag add <名前> [--color <hex>]` | タグを新規作成（別名 `new`） |
| `tag delete <名前>` | タグを削除し、全バージョンのエントリから除去（別名 `remove` / `rm`） |
| `tag rename <旧名> <新名>` | タグをリネームし、全バージョンのエントリへ同期（別名 `mv`） |
...
| `Brief` | `string` | 概要 |
| `Detail` | `string` | 詳細 |
| `Deadline` | `string` (ISO 8601) | 期限（時刻含む） |
| `CompletedAt` | `string` (ISO 8601) | 完了時刻（未完了時は空） |
| `CreatedAt` | `string` (ISO 8601) | 作成時刻 |
| `UpdatedAt` | `string` (ISO 8601) | 最終更新時刻 |
| `Status` | `string` | 状態：`Unfinished` / `Finished` |
